...
const Texto         = require('./docx-texto');
const {
...
// ── Gênero do cliente ────────────────────────────────────────────────────────
// Vem do seletor obrigatório da ficha. Não se tenta adivinhar: um documento
// com a concordância errada é pior do que um aviso pedindo o dado.
function generoDoCliente(d) {
  const g = String((d && d.genero) || '').trim().toUpperCase();
  if (g.startsWith('F')) return 'F';
  if (g.startsWith('M')) return 'M';
  return null;
}

// Monta o endereço a partir dos campos da ficha, sem deixar "/" ou vírgulas
// soltas quando algum campo fica em branco.
function montarEndereco(d) {
  const cidadeUf = [d.cidade, d.uf].map((x) => String(x || '').trim()).filter(Boolean).join('/');
  const partes = [d.rua, d.numEnd, d.complemento, d.bairro, cidadeUf]
    .map((x) => String(x || '').trim()).filter(Boolean);
  if (partes.length) return partes.join(', ');
  const livre = String(d.endereco || '').trim();
  return /[\p{L}\d]/u.test(livre) ? livre : '';
}

// ── 1) Documentos a partir dos templates timbrados ───────────────────────────
const PARTES_COM_TEXTO = /^word\/(document|header\d*|footer\d*)\.xml$/;

function aplicarNasPartes(zip, fn) {
  Object.keys(zip.files)
    .filter((nome) => PARTES_COM_TEXTO.test(nome))
    .forEach((nome) => zip.file(nome, fn(zip.file(nome).asText())));
}

function preencherTemplate(nomeArq, dados, genero) {
  ...
  // O marca-texto amarelo servia para o funcionário saber o que trocar à mão.
  // Com o preenchimento automático ele só atrapalha — sai de qualquer modelo.
  aplicarNasPartes(zip, Texto.removerRealces);
  ...
  // Depois de preenchido: "portador(a)", "o(a) Contratante" e cia. viram a
  // forma certa para o cliente; vírgulas de campos vazios somem.
  const saida = doc.getZip();
  aplicarNasPartes(saida, (xml) => Texto.substituirNosParagrafos(xml, [
    Texto.trocaDeGenero(genero === 'F'),
    Texto.TROCA_VIRGULAS_VAZIAS,
  ]));

  return saida.generate({ type: 'nodebuffer', compression: 'DEFLATE' });
}

function dadosDoTemplate(d, agora) {
  return {
    nomeCliente:   String(d.nomeCliente || '').trim().toUpperCase(),
    // Em minúsculas no meio da qualificação: "brasileiro, casado, pedreiro"
    nacionalidade: String(d.nacionalidade || 'brasileiro(a)').trim().toLowerCase(),
    estadoCivil:   String(d.estadoCivil || '').trim().toLowerCase(),
...
async function gerarFicha(d, protocolo, agora) {
  ...
  const genero = generoDoCliente(d);
  c.push(tabelaPares([
    ['Nome completo', d.nomeCliente],
    ['Gênero', genero === 'F' ? 'Feminino' : genero === 'M' ? 'Masculino' : ''],
    ['CPF', d.cpf],
    ['RG', d.rg],
...
async function gerarTodos(d, protocolo, agora = new Date()) {
  ...
  if (!genero) {
    throw new Error('Informe se o cliente é homem ou mulher (campo "Gênero" na ficha): '
      + 'contrato, procuração, declaração, termo e carta de boas-vindas mudam conforme o gênero.');
  }
  ...
  // Ordem definida pelo escritório em 10/09/2026
  return [
    { nome: `0_Boas_Vindas_${base}.pdf`,        buffer: await BoasVindas.gerarBoasVindas(d) },
    { nome: `1_Procuracao_${base}.docx`,        buffer: preencherTemplate('TEMPLATE_PROCURACAO.docx', dt, genero) },
    { nome: `2_Declaracao_Hipossuficiencia_${base}.docx`, buffer: preencherTemplate('TEMPLATE_DECLARACAO_HIPOSSUFICIENCIA.docx', dt, genero) },
    { nome: `3_Termo_Ciencia_${base}.docx`,     buffer: preencherTemplate('TEMPLATE_TERMO_CIENCIA.docx', dt, genero) },
    { nome: `4_Ficha_Atendimento_${base}.docx`, buffer: await gerarFicha(d, protocolo, agora) },
    { nome: `5_Resumo_Juridico_${base}.docx`,   buffer: await gerarResumo(d, protocolo, agora) },
    { nome: `6_Contrato_Honorarios_${base}.docx`, buffer: preencherTemplate('TEMPLATE_CONTRATO_DE_HONORARIOS.docx', dt, genero) },
  ];
}

// Campos da qualificação que, em branco, deixam lacunas nos documentos
function lacunasDaQualificacao(d) {
  const faltam = [];
  if (!String(d.cpf || '').trim()) faltam.push('CPF');
  if (!String(d.rg || '').trim()) faltam.push('RG');
  if (!String(d.estadoCivil || '').trim()) faltam.push('estado civil');
  if (!String(d.profissao || '').trim()) faltam.push('profissão');
  if (!montarEndereco(d)) faltam.push('endereço');
  if (!String(d.cep || '').trim()) faltam.push('CEP');
  return faltam;
}

// Documentos que vão para assinatura no ZapSign (ficha, resumo e carta são
// internos). Reconhecidos pelo nome, não pelo número — a ordem pode mudar.
const PARA_ASSINATURA = [/_Procuracao_/, /_Declaracao_/, /_Termo_Ciencia_/, /_Contrato_/];
...
module.exports = {
  ...
  generoDoCliente, montarEndereco, lacunasDaQualificacao,
};
