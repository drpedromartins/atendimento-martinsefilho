...
// ── Gênero do cliente ────────────────────────────────────────────────────────
// Usa o campo do formulário; se estiver vazio, deduz do estado civil.
function ehFeminino(d) {
  const g = String(d.genero || '').trim().toLowerCase();
  if (g.startsWith('f')) return true;
  if (g.startsWith('m')) return false;

  const ec = String(d.estadoCivil || '').trim().toLowerCase();
  if (/(casada|solteira|divorciada|viúva|viuva|separada|companheira)/.test(ec)) return true;
  if (/(casado|solteiro|divorciado|viúvo|viuvo|separado|companheiro)/.test(ec)) return false;

  return null; // indefinido — o texto usa a forma neutra "bem-vindo(a)"
}
...
async function gerarBoasVindas(d) {
  ...
  // guiá-lo). Sem gênero definido, usa o feminino.
  const modelo = path.join(TEMPLATES, `BOAS_VINDAS_${fem === false ? 'M' : 'F'}_LIMPO.pdf`);
