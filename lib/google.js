...
function getAuth() {
  const c = credenciais();
  const usuario = (process.env.IMPERSONATE_USER || '').trim();
  if (usuario) {
    // Delegação em todo o domínio: a conta de serviço age como o usuário do
    // Workspace, então os arquivos nascem na cota dele (e não na da conta de
    // serviço, que não tem cota própria de armazenamento).
    return new google.auth.JWT({
      email: c.client_email, key: c.private_key, scopes: SCOPES, subject: usuario,
    });
  }
...
module.exports = {
  MIME_DOCX, MIME_PASTA, CABECALHO,
  getAuth, emailContaServico,
